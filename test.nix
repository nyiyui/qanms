...
in {
  ...
  coordServerIntegration-single = lib.runTest (let
    ...
    client1TokenHash =
      "qrystalcth_30e72874f2598c1ad8020507182a4f57a7304806947b677b69c7d76a88003bc6";
    client1Config = pkgs.writeText "client1config.json" (builtins.toJSON {
      ...
      Token =
        "qrystalct_ZXcX7NyjY2aqiy5bb7Oe952QSCsVxzh2FU2ahvaPiHZPJaWeN+Xi59HHvqTDT0Tyy7IOhzC9Uc3Nn7dQ+8GhbQ==";
      Network = "wiring";
      Device = "client1";
      PrivateKey = client1PrivateKey;
    });
    tokens = {
      ${client1TokenHash} = { Identities = [[ "wiring" "client1" ]]; };
    };
    spec.Networks = [{
      Name = "wiring";
      Devices = [
  ...
  in {
    ...
    nodes.client2 =
      { pkgs, ... }: { # wg peer setup using NixOS's wireguard module
        imports = [ base ];
        networking.wireguard.interfaces = {
          wiring = {
            ips = [ "10.10.0.2/32" ];
  ...
  coordServerIntegration-double = lib.runTest (let
    ...
    client2TokenHash =
      "qrystalcth_4f4a908fbcc2f13f45ee71b438efa4df982f99526b085a42fbe05b019056af9e";
    client2Config = pkgs.writeText "client2config.json" (builtins.toJSON {
      MachineJSONPath = "/tmp/machine.json";
      BaseURL = "http://server:${builtins.toString serverPort}";
      Token =
        "qrystalct_wrN7qG37s1KeewvlafCI7GXPC71Jx6DZQAexAJTcfMHRveN7CCMebxo5VIfZxP0YQqSd79rblAgkZkZjJENRMQ==";
      Network = "wiring";
      Device = "client2";
      PrivateKey = client2PrivateKey;
    });
    tokens = {
      ${client1TokenHash} = { Identities = [[ "wiring" "client1" ]]; };
      ${client2TokenHash} = { Identities = [[ "wiring" "client2" ]]; };
    };
    spec.Networks = [{
      Name = "wiring";
      Devices = [
  ...
  coordServerIntegration-continuous-single = lib.runTest (let
    etc = self.outputs.packages.${system}.etc;
    serverPort = 39390;
    client1PrivateKey = "WMRg3WjzE0ZLuP4bAWtvcrh/Tw23MR3kv4VjpAQQz04=";
    client1PublicKey = "0LrS7ekXRHD8pLEimzLfeLlKyPprJR9oJwdAMOGhtU0=";
  ...
  coordServerIntegration-continuous-single-dns = lib.runTest (let
    etc = self.outputs.packages.${system}.etc;
    serverPort = 39390;
    client1PrivateKey = "WMRg3WjzE0ZLuP4bAWtvcrh/Tw23MR3kv4VjpAQQz04=";
    client1PublicKey = "0LrS7ekXRHD8pLEimzLfeLlKyPprJR9oJwdAMOGhtU0=";
    ...
    client1DNSConfig = pkgs.writeText "client1config.json" (builtins.toJSON {
      Parents = [
        { Suffix = ".qrystal.internal"; }
        { Suffix = "client1.nyiyui.ca"; Network = "wiring"; Device = "client1"; }
        { Suffix = "client2.nyiyui.ca"; Network = "wiring"; Device = "client2"; }
      ];
    });
    tokens = {
      ${client1TokenHash} = { Identities = [[ "wiring" "client1" ]]; };
  ...
  in {
    ...
    in { # wg peer setup using test-device
      imports = [ base ];
